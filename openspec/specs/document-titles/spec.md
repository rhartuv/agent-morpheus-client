...
Tab titles SHALL be built only through `pageTitles.ts` helpers, applied with `useDocumentTitle`, and SHALL end with ` | ExploitIQ` (via `withAppTitle` / `DOCUMENT_TITLE_APP_NAME`).

#### Scenario: Conventions
- **WHEN** any page sets the document title
- **THEN** it uses `pageTitles.ts` plus `useDocumentTitle` and the string ends with ` | ExploitIQ`
...
Titles MUST still follow **`document-titles`** global conventions (constructed via **`pageTitles.ts`** / **`useDocumentTitle`** and the **`ExploitIQ`** suffix).
...
- **AND** the suffix follows **`pageTitles.ts`** conventions with the **ExploitIQ** app suffix
