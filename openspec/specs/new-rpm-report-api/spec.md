...
- **WHEN** a client calls `POST /api/v1/reports/new-rpm-report` with `name`, `version`, `release`, and `cveId` valid per other rules but `arch` is a non-empty string after trim that is not one of **`x86_64`**, **`amd64`**, **`aarch64`**, **`arm64`**, **`ppc64le`**, **`s390x`**, **`i686`**
- **THEN** the API returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
...
The new endpoint SHALL be documented in the application OpenAPI/Swagger specification with the JSON request schema (`name`, `version`, `release`, `arch`, `cveId`), where **`arch`** is documented with an **`enum`** (or equivalent OpenAPI enumeration) listing exactly **`x86_64`**, **`amd64`**, **`aarch64`**, **`arm64`**, **`ppc64le`**, **`s390x`**, **`i686`**, response schema for HTTP 202 (aligned with existing report creation responses), documented HTTP **400** response describing the **field-name-to-message object** matching upload-spdx-style validation errors, and HTTP **429** consistent with queued report submission.
